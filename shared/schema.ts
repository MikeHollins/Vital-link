...
// Consent management table for privacy compliance
export const consentRecords = pgTable(
  "consent_records",
  {
    ...
    dataTypes: jsonb("data_types").notNull(),
  ...
  (table) => [
    ...
    index("consent_active_idx").on(table.isActive),
  ]
);
...
export const verificationRequests = pgTable(
  "verification_requests",
  {
    ...
    userId: varchar("user_id").notNull().references(() => users.id),
    proofId: integer("proof_id").references(() => zkHealthProofs.id),
  ...
  (table) => [
    ...
    index("verification_requester_idx").on(table.requesterId),
    index("verification_status_idx").on(table.status),
  ]
);

// Environmental metadata for constraint adaptation
export const environmentalData = pgTable(
  ...
  (table) => [
    index("env_user_idx").on(table.userId),
    index("env_timestamp_idx").on(table.timestamp),
  ]
);
