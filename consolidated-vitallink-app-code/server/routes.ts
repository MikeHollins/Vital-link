...
import { platformLogoMatcher } from "./platform-logo-matcher";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Platform Logo Matching Endpoints
  app.get('/api/platforms/logos/supported', async (req, res) => {
    try {
      const platforms = platformLogoMatcher.getSupportedPlatforms();
      res.json({
        success: true,
        platforms
      });
    } catch (error) {
      console.error('Error fetching logo platforms:', error);
      res.status(500).json({ error: 'Unable to fetch platform logo information' });
    }
  });

  app.post('/api/platforms/logos/match', isAuthenticated, async (req: any, res) => {
    try {
      const { platformName } = req.body;
      ...
      const logoInfo = await platformLogoMatcher.matchPlatformLogo(platformName);
      ...
      res.status(500).json({ 
        error: 'Logo matching service temporarily unavailable' 
      });
    }
  });

  app.post('/api/platforms/logos/batch-match', isAuthenticated, async (req: any, res) => {
    try {
      const { platformNames } = req.body;
      ...
      const logoInfos = await platformLogoMatcher.batchMatchPlatformLogos(platformNames);
      
      res.json({
        success: true,
        logoInfos,
        matched: logoInfos.length,
        requested: platformNames.length
      });

    } catch (error) {
      console.error('Batch logo matching error:', error);
      res.status(500).json({ 
        error: 'Batch logo matching service temporarily unavailable' 
      });
    }
  });
  ...
  app.post('/api/platforms/logos/generate-variants', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const logoInfo = await platformLogoMatcher.matchPlatformLogo(platformName);
      const variants = await platformLogoMatcher.generateLogoVariants(logoInfo);
  ...
  app.post('/api/platforms/logos/validate-compliance', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const logoInfo = await platformLogoMatcher.matchPlatformLogo(platformName);
      const compliance = await platformLogoMatcher.validateAppStoreCompliance(logoInfo);
