...
export class AICorrelationAnalyzer {
  ...
  async analyzeStepCorrelations(healthData: HealthMetricData[]): Promise<CorrelationAnalysis[]> {
    ...
    const metrics = [
      { name: 'remSleep', data: healthData.map(d => d.remSleep || 0), unit: 'hours' },
      { name: 'screenTime', data: healthData.map(d => d.screenTime || 0), unit: 'hours' },
      { name: 'heartRate', data: healthData.map(d => d.heartRate || 0), unit: 'bpm' },
      { name: 'mood', data: healthData.map(d => d.mood || 0), unit: 'score' },
      { name: 'stressLevel', data: healthData.map(d => d.stressLevel || 0), unit: 'level' }
    ];
